...
void SolverBase::init()
{
    ...
    {
        if (d == 2) {
            VQ.reset(new Elastic2DP1P1::FunctionSpace(mesh));
    ...
    {
        if (d == 2) {
            J_f.reset(new Fluid2DP1P1::JacobianForm(VQ,VQ));

            F_f.reset(new Fluid2DP1P1::ResidualForm(VQ));
            ...
            if(_parameters["ElasticModel"].value_str() =="Linear")
            {
                    ...
                    J_s.reset(new Elastic2DP1P1::JacobianForm(VQ,VQ));
                    F_s.reset(new Elastic2DP1P1::ResidualForm(VQ));
    ...
    // variable
    v_p.reset(new Function(VQ));  //!< velocity for fluid mesh
    //Function ini_p(Q);
    //ini_p = Constant(13330.0);
    //assign(reference_to_no_delete_pointer( (*v_p)[1]), reference_to_no_delete_pointer(ini_p));

    u.reset(new Function(V));  //!< displacement
    u0.reset(new Function(V)); //!< displacement at last time-step 

    uA.reset(new Function(V));  //!< displacement for smoothing
    uA0.reset(new Function(V)); //!< displacement for smoothing at last time-step
    uA1.reset(new Function(V)); //!< displacement for smoothing at last time-step
    w.reset(new Function(V));  //!< displacement for smoothing at last time-step

    v.reset(new Function(V)); //!< velocity
    v0.reset(new Function(V)); //!< velocity
    v1.reset(new Function(V)); //!< velocity
    dv.reset(new Function(V)); //!< velocity

    p.reset(new Function(Q));
...
void SolverBase::step()
{
    ...
    for(int i=0; i<iter_max; i++)
    {
        ...
        *(uA1->vector()) = *(uA->vector());
        ...
        newton_solver->solve(*nonlinear_problem, *(v_p->vector()));
        std::cout << "Newtonsolver done" <<std::endl;


        assign(v,reference_to_no_delete_pointer( (*v_p)[0]));
        assign(p, reference_to_no_delete_pointer((*v_p)[1]));
        ...
        *mesh = *mesh0;
        ...
        meshsmoother->smoothing(mesh, *u, *uA);
        *(w->vector()) = *(uA->vector());
        *(w->vector()) -= *(uA0->vector());
        *(w->vector()) /= dt;
#ifdef _DEBUG
...
void SolverBase::pressure_dirichlet()
{
    ...
    const GenericDofMap& dofmap = *(VQ->dofmap());
...
//-----------------------------------------------------------------------------
    void
SolverBase::NonlinearDiscreteProblem::J(GenericMatrix& A,
